...
// $sql2="select Count(*) from event_booking where buy=1 and customer_id=$customer_id";
...
{
	$id=$_POST['result'];
	$sql="update event_booking set status=0, paystatus= 'REFUND or Cancelled',reference ='CANCELLED', remark='Order has been Cancelled by User' where event_id=$id and booking_id= (select booking_id from event_booking where event_id=$id and status=1 Limit 1)";
	$result=mysqli_query($con,$sql)or die($sql);

$sql2 ="update payment set status = 0,commision=0 where event_id = $id and customer_id=customer_id";
...
<body id="page-top">
    ...
    <div id="wrapper">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                              ...
                              <img class="img-profile rounded-circle"
                                   src="img/undraw_profile.svg"><!-- evde vene pro pic vekkam -->
                            </a>
		...
		<div class="container">
			...
			<center><h2 class="heading-agileinfo"><br><br>MY ORDERS<span><hr>Events is a professionally managed Event</span></h2></center>
			
            
	<?php
		$sql2="select * from event_booking where buy=1 and status=1  and customer_id=$customer_id";
  		$result1=mysqli_query($con,$sql2)or die($sql2);
...
?>
  		<div class="col-md-12">
            ...
            if ($cart>=0) 
                // evd oru prlm varum enne thonnunnu....
             {
			...
			$sql2="select * from events where event_id=$ide";
  			...
  			foreach($result2 as $rowe)
  			{
				...
				if($row['status']==1){
					?>
				  <input type="submit" class="btn btn-primary" name="remove" onclick="one('<?php echo $row['event_id'] ?>')" value="Cancel Order" style=" float: right;"> 
                  
             <!--      <input type="submit" class="btn btn-warning" name="remove" onclick="one('<?php echo $row['event_id'] ?>')" value="Change Date" style=" float: right;"> -->
...
<?php
            ...
            ?>
						<table>
						<tr ><td ><strong> Event type</strong></td><td > : <?php echo $rowe['event_type'] ?></td>
                        <td ><strong> Venue</strong></td><td > : <?php echo $row['venue'] ?></td></tr>
                        <tr ><td ><strong> time</strong></td><td > : <?php echo $row['time'] ?></td>
                        <td ><strong> Event-date</strong></td><td > : <?php echo $row['date'] ?></td></tr>
                        <tr><td ><strong> landmark</strong></td><td > : <?php echo $row['landmark'] ?></td>
                        <td ><strong>Booked-Date</strong></td><td > : <?php echo $row['datebooked'] ?></td></tr>
                        <tr><td ><strong> Payment Method</strong></td><td > : <?php echo $row['payment'] ?></td></tr>
                        <tr><td ><strong> Status</strong></td><td > : <?php echo $row['remark'] ?></td></tr>
                      </table>
                      <br>
                      <br>
